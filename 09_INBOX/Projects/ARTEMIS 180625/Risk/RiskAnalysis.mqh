...
private:
    ...
    MoneyManager* m_money_manager;
    TradeManager* m_trade_manager;
    RiskSystem* m_risk_system2;
    RiskControl* m_risk_control;
...
public:
    ...
    bool Initialize(string symbol, ENUM_TIMEFRAMES timeframe, double risk_per_trade = 0.02, double max_daily_risk = 0.05, double max_drawdown = 0.15, int atr_period = 14) {
        if(m_is_initialized) {
            Print("RiskAnalysis já inicializado");
            return false;
        }
        ...
        m_money_manager = new MoneyManager();
        if(!m_money_manager.Initialize(symbol, timeframe)) {
            Print("Erro ao inicializar MoneyManager");
            IndicatorRelease(m_atr_handle);
            m_atr_handle = INVALID_HANDLE;
            m_timeframe_hierarchy.Release();
            delete m_timeframe_hierarchy;
            ...
            delete m_risk_manager;
            m_risk_manager = NULL;
            delete m_money_manager;
            m_money_manager = NULL;
            return false;
        }
        
        m_trade_manager = new TradeManager();
        if(!m_trade_manager.Initialize(symbol, timeframe)) {
            Print("Erro ao inicializar TradeManager");
            IndicatorRelease(m_atr_handle);
            m_atr_handle = INVALID_HANDLE;
            m_timeframe_hierarchy.Release();
            delete m_timeframe_hierarchy;
            ...
            delete m_risk_manager;
            m_risk_manager = NULL;
            m_money_manager.Release();
            delete m_money_manager;
            m_money_manager = NULL;
            delete m_trade_manager;
            m_trade_manager = NULL;
            return false;
        }
        
        m_risk_system2 = new RiskSystem();
        if(!m_risk_system2.Initialize(symbol, timeframe)) {
            Print("Erro ao inicializar RiskSystem");
            IndicatorRelease(m_atr_handle);
            m_atr_handle = INVALID_HANDLE;
            m_timeframe_hierarchy.Release();
            delete m_timeframe_hierarchy;
            ...
            delete m_money_manager;
            m_money_manager = NULL;
            m_trade_manager.Release();
            delete m_trade_manager;
            m_trade_manager = NULL;
            delete m_risk_system2;
            m_risk_system2 = NULL;
            return false;
        }
        
        m_risk_control = new RiskControl();
        if(!m_risk_control.Initialize(symbol, timeframe)) {
            Print("Erro ao inicializar RiskControl");
            IndicatorRelease(m_atr_handle);
            m_atr_handle = INVALID_HANDLE;
            m_timeframe_hierarchy.Release();
            delete m_timeframe_hierarchy;
            ...
            delete m_trade_manager;
            m_trade_manager = NULL;
            m_risk_system2.Release();
            delete m_risk_system2;
            m_risk_system2 = NULL;
            delete m_risk_control;
            m_risk_control = NULL;
            return false;
        }
        
        m_risk_metrics = new RiskMetrics();
        if(!m_risk_metrics.Initialize(symbol, timeframe)) {
            Print("Erro ao inicializar RiskMetrics");
            IndicatorRelease(m_atr_handle);
            m_atr_handle = INVALID_HANDLE;
            m_timeframe_hierarchy.Release();
            delete m_timeframe_hierarchy;
            ...
            delete m_risk_system2;
            m_risk_system2 = NULL;
            m_risk_control.Release();
            delete m_risk_control;
            m_risk_control = NULL;
            delete m_risk_metrics;
            m_risk_metrics = NULL;
            return false;
        }
    ...
    void Release() {
        ...
        if(m_risk_metrics != NULL) {
            m_risk_metrics.Release();
            delete m_risk_metrics;
            m_risk_metrics = NULL;
        }
