//+------------------------------------------------------------------+
//| AdaptiveRiskSystem.mqh - Sistema de Gestão de Risco Adaptativo   |
//| Versão Corrigida                                                 |
//+------------------------------------------------------------------+
...
public:
    ...
    bool Initialize(string symbol, ENUM_TIMEFRAMES timeframe, double risk_per_trade = 0.02, double max_daily_risk = 0.05, double max_drawdown = 0.15, int atr_period = 14) {
        if(m_is_initialized) {
            Print("AdaptiveRiskSystem já inicializado");
            return false;
        }
        ...
        // Inicializar MA rápida
        m_ma_fast_handle = iMA(m_symbol, m_timeframe, 10, 0, MODE_EMA, PRICE_CLOSE);
