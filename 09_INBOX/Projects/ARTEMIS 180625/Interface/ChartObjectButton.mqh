...
class ChartObjectButton : public CChartObject {
private:
    string m_text;
    color m_color;
    ...
    int m_y_size;
    bool m_state;
    ENUM_BORDER_TYPE m_border_type;
    bool m_is_created;
...
public:
    ChartObjectButton(long chart_id, string name) : CChartObject(chart_id, name) {
        m_text = "";
        m_color = clrBlack;
        ...
        m_x_distance = 0;
        m_y_distance = 0;
        m_x_size = 50;  // Tamanho padrão
        m_y_size = 30;  // Tamanho padrão
        m_state = false;
        m_border_type = BORDER_FLAT;
        m_is_created = false;
    }
    
    bool Create(long chart_id, int x, int y) {
        if(m_is_created) {
            Print("Botão já existe: ", m_name);
            return false;
        }
        ...
        if(!CChartObject::Create(chart_id, OBJ_BUTTON, 0, x, y)) {
            Print("Erro ao criar botão: ", GetLastError());
            return false;
        }
        ...
        if(!BorderType(m_border_type)) return false;
    ...
    bool BorderType(ENUM_BORDER_TYPE type) {
        if(!m_is_created) return false;
        m_border_type = type;
        return ObjectSetInteger(m_chart_id, m_name, OBJPROP_STYLE, type);
    }
