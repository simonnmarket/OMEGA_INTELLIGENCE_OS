...
class ChartObjectLabel : public CChartObject {
private:
    string m_text;
    color m_color;
...
public:
    ChartObjectLabel(long chart_id, string name) : CChartObject(chart_id, name) {
        m_text = "";
        m_color = clrBlack;
    ...
    bool Create(long chart_id, int x, int y) {
        ...
        if(!CChartObject::Create(chart_id, OBJ_LABEL, 0, x, y)) {
