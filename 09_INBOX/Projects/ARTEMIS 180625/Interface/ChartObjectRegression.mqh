...
class ChartObjectRegression : public CChartObject {
private:
    datetime m_time1;
    datetime m_time2;
    double m_price1;
...
public:
    ChartObjectRegression(long chart_id, string name) : CChartObject(chart_id, name) {
        m_time1 = 0;
        m_time2 = 0;
        m_price1 = 0;
        m_price2 = 0;
    ...
    bool Create(long chart_id, datetime time1, double price1, datetime time2, double price2) {
        if(m_is_created) {
            Print("Linha de regressão já existe: ", m_name);
            return false;
        }
        ...
        if(!CChartObject::Create(chart_id, OBJ_REGRESSION, 0, time1, price1)) {
            Print("Erro ao criar linha de regressão: ", GetLastError());
            return false;
        }
