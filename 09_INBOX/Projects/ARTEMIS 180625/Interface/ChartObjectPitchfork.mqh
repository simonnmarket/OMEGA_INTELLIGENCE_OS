...
class ChartObjectPitchfork : public CChartObject {
private:
    datetime m_time1;
    datetime m_time2;
    datetime m_time3;
...
public:
    ChartObjectPitchfork(long chart_id, string name) : CChartObject(chart_id, name) {
        m_time1 = 0;
        m_time2 = 0;
        m_time3 = 0;
        m_price1 = 0;
    ...
    bool Create(long chart_id, datetime time1, double price1, datetime time2, double price2, datetime time3, double price3) {
        if(m_is_created) {
            Print("Pitchfork já existe: ", m_name);
            return false;
        }
        
        if(!ValidateTime(time1) || !ValidateTime(time2) || !ValidateTime(time3)) {
            Print("Tempo inválido");
            return false;
        }
        
        if(!ValidatePrice(price1) || !ValidatePrice(price2) || !ValidatePrice(price3)) {
            Print("Preço inválido");
            return false;
        }
        
        if(!CChartObject::Create(chart_id, OBJ_PITCHFORK, 0, time1, price1)) {
            Print("Erro ao criar Pitchfork: ", GetLastError());
            return false;
        }
        ...
        if(!Time3(time3)) return false;
        if(!Price3(price3)) return false;
    ...
    bool Time3(datetime time) {
        if(!m_is_created) return false;
        if(!ValidateTime(time)) {
            Print("Tempo 3 inválido: ", time);
            return false;
        }
        m_time3 = time;
        return ObjectSetInteger(m_chart_id, m_name, OBJPROP_TIME3, time);
    }
    
    bool Price3(double price) {
        if(!m_is_created) return false;
        if(!ValidatePrice(price)) {
            Print("Preço 3 inválido: ", price);
            return false;
        }
        m_price3 = price;
        return ObjectSetDouble(m_chart_id, m_name, OBJPROP_PRICE3, price);
    }
    ...
    bool Move(datetime time1, double price1, datetime time2, double price2, datetime time3, double price3) {
        if(!m_is_created) return false;
        if(!ValidateTime(time1) || !ValidateTime(time2) || !ValidateTime(time3)) {
    ...
    datetime GetTime3() {
        if(!m_is_created) return 0;
        return (datetime)ObjectGetInteger(m_chart_id, m_name, OBJPROP_TIME3);
    }
    
    double GetPrice3() {
        if(!m_is_created) return 0;
        return ObjectGetDouble(m_chart_id, m_name, OBJPROP_PRICE3);
    }
