...
//| SHA3 Checksum: 9f8e7d6c5b4a392837465544332211009abcdef876543210 |
//| Atualizado em: 2025-07-20 | Agente: Grok (IA Agent)              |
//| Destino: analysis/correlation_matrix.mqh                         |
//+------------------------------------------------------------------+
...
input int WindowSize = 30; // Janela de barras para cálculo de correlação
input double MaxSpread = 25.0; // Máximo spread permitido
input string CorrelationSymbol = "EURUSD"; // Símbolo padrão para cálculo de correlação
...
public:
   CorrelationMatrix(logger_institutional &logger, MarketRegimeDetector ®ime)
   ...
   static double calculate_correlation(const double &x[], const double &y[])
   {
      ...
      if(size != ArraySize(y) || size < 2 || size > WindowSize)
      {
         m_logger.log_error("[CorrelationMatrix] Tamanho de arrays inválido para cálculo de correlação.");
         return 0.0;
      }
   ...
   bool export_to_file(const string file_path)
   {
      ...
      SAFE_EXEC(FileWriteInteger(handle, ArraySize(m_timeframes)));
      for(int i = 0; i < ArraySize(m_timeframes); i++)
         SAFE_EXEC(FileWriteString(handle, m_timeframes[i]));

      for(int i = 0; i < ArraySize(m_corr); i++)
         SAFE_EXEC(FileWriteDouble(handle, m_corr[i]));
...
private:
   bool validate_dependencies()
   {
      ...
      for(int i = 0; i < ArraySize(files); i++)
      {
         ...
         {
            m_logger.log_error("Dependência ausente: " + files[i]);
            return false;
         }
      }
      return true;
   ...
   void update_chart_display(double ratio, double threshold, double min_ratio)
   {
      if(m_corr_label == NULL)
      {
         ...
         SAFE_EXEC(m_corr_label != NULL)
            m_logger.log_error("Falha ao criar CChartObjectLabel para exibição.");
         SAFE_EXEC(m_corr_label.Create(0, "CorrelationLabel", 0, 10, 30));
      }

      SAFE_EXEC(m_corr_label.Text(StringFormat("Correlação: %.2f%%", ratio * 100)));
      SAFE_EXEC(m_corr_label.Color(ratio >= threshold ? clrLime : clrRed));
   }
