...
public:
   ...
   //| CONSTRUTOR                                                   |
   //+--------------------------------------------------------------+
   CGenesisTradeSignalUtils(CGenesisUtils &logger, string symbol = _Symbol) :
      m_logger(&logger),
      m_symbol(symbol),
...
private:
   ...
   void CreateSidebarLabels()
   {
      for(int i = 0; i < ArraySize(m_modules); i++)
      {
         ...
         ObjectSetString(0, label_name, OBJPROP_TEXT, m_modules[i].display_name);
         ObjectSetInteger(0, label_name, OBJPROP_COLOR, m_modules[i].is_active ? m_active_color : m_inactive_color);
         ObjectSetInteger(0, label_name, OBJPROP_XDISTANCE, m_x_position);
         ObjectSetInteger(0, label_name, OBJPROP_YDISTANCE, m_y_position + (i * m_label_height));
...
public:
   ...
   //| Construtor                                                   |
   //+--------------------------------------------------------------+
   CGenesisSidebarManager(CGenesisUtils &logger, int x_pos = 10, int y_pos = 50) :
      m_logger(&logger),
      m_x_position(x_pos),
      m_y_position(y_pos),
      m_label_height(25),
      ...
      m_inactive_color(clrGray)
   {
      InitializeModules();
      CreateSidebarLabels();
      Print("[SIDEBAR] Sidebar inicializado com " + IntegerToString(ArraySize(m_modules)) + " módulos");
   }
   ...
   void UpdateModuleStatus(ENUM_SIDEBAR_MODULES module_id, bool is_active)
   {
      for(int i = 0; i < ArraySize(m_modules); i++)
      {
         if(m_modules[i].module_id == module_id)
         {
            ...
            string label_name = "Sidebar_" + m_modules[i].module_name;
            ObjectSetInteger(0, label_name, OBJPROP_COLOR, is_active ? m_active_color : m_inactive_color);
            break;
         }
      }
   }
   ...
   void ListAllModules()
   {
      Print("[SIDEBAR] Lista de módulos do sidebar:");
      for(int i = 0; i < ArraySize(m_modules); i++)
      {
         string status = m_modules[i].is_active ? "ATIVO" : "INATIVO";
         Print(StringFormat("[SIDEBAR] %d. %s (%s) - %s", 
                           i + 1, 
   ...
   ~CGenesisSidebarManager()
   {
      ...
      Print("[SIDEBAR] Sidebar encerrado");
   }
};
