...
try:
    ...
    if os.path.exists(base_path):
        ...
        raise ImportError("ncnt_base.py não encontrado")
...
class NCNTModule(NCNTBaseModule):
        ...
        try:
            ...
            if self.COMPLIANCE_REQUIRED:
                ...
                if compliance_result["overall_status"] == "VIOLATION":
                    self.logger.error(f"❌ Violações de compliance: {compliance_result['violations']}")
    ...
    def _calculate_module_checksum(self) -> str:
        ...
        try:
            ...
            self.logger.debug(f"🔒 Checksum do módulo: {self.module_checksum[:12]}...")
            self.logger.debug(f"🔒 Checksum da instância: {self.instance_checksum[:12]}...")
    ...
    def _register_with_genesis(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(genesis_path):
                self.logger.warning(f"⚠️ Genesis Includes não encontrado em {genesis_path}")
                return False
            ...
            self.logger.info(f"✅ Módulo {self.module_name} registrado no Genesis")
            return True
    ...
    def _validate_dependencies(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(genesis_path):
                self.logger.warning("⚠️ Genesis não encontrado - pulando validação de dependências")
                return True
            ...
            self.logger.warning(f"⚠️ Erro ao validar dependências: {e}")
    ...
    def _establish_neural_connections(self) -> bool:
        ...
        try:
            ...
            for dep_name in self.required_modules:
                try:
                    ...
                    if dep_instance:
                        ...
                        self.logger.info(f"🔗 Conexão neural estabelecida: {self.module_name} -> {dep_name}")
                    
                except Exception as e:
                    self.logger.warning(f"⚠️ Falha ao conectar com {dep_name}: {e}")
            ...
            self.logger.error(f"❌ Erro ao estabelecer conexões: {e}")
            return False
    ...
    def _emit_completion_signal(self):
        ...
        # SINAL PRINCIPAL (sempre visivel - SEU PEDIDO)
        print(f"\n{'='*70}")
        ...
        print(f"   CONEXÕES: {len(self.neural_connections)} neural connection(s)")
        print(f"   COMPLIANCE: {self.vitals.compliance_status}")
        print(f"   TEMPO BOOT: {elapsed_ms:.2f} ms")
        print(f"{'='*70}\n")
        
        # Log estruturado
        self.logger.info(f"✅ Módulo {self.module_name} inicializado em {elapsed_ms:.2f}ms")
        self.logger.info(f"📊 Checksum: {self.module_checksum[:16]}...")
        self.logger.info(f"🔗 Conexões: {len(self.neural_connections)} estabelecidas")
        self.logger.info(f"⚖️  Compliance: {self.vitals.compliance_status}")
    ...
    def _emit_failure_signal(self, error):
        ...
        print(f"   ID: {self.module_id}")
        print(f"   TEMPO ATÉ FALHA: {elapsed_ms:.2f} ms")
        print(f"{'='*70}\n")
        
        # Log de erro
        self.logger.error(f"❌ Falha na inicialização após {elapsed_ms:.2f}ms: {error}")
    ...
    def send_neural_signal(self, target_module: str, signal: Dict) -> bool:
        ...
        if target_module not in self.connected_modules:
            self.logger.warning(f"⚠️ Módulo {target_module} não está conectado")
            return False
    ...
    async def process_transmission(self, transmission: NCNTTransmission) -> Optional[NCNTTransmission]:
        ...
        self.logger.debug(f"📨 Transmissão recebida: {transmission.transmission_id}")
        return None
...
class RiskControlsModule(NCNTModule):
    ...
    def _establish_neural_connections(self) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"⚠️ Conexões específicas incompletas: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if success:
            print(f"✅ Módulo base inicializado")
            ...
            print("❌ Falha na inicialização do módulo base")
        ...
        if risk_success:
            print(f"✅ RiskControlsModule inicializado")
