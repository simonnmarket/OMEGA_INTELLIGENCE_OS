#!/usr/bin/env python3
"""
Script para restaurar o relatório executivo corrompido
"""
...
def restore_executive_report():
    """Restaura o relatório executivo a partir do relatório ultra completo"""
    
    # Ler o relatório ultra completo
    ultra_report_path = Path(__file__).parent.parent / "05-Documentacao" / "Audit-Reports" / "AURORA_ULTRA_COMPLETE_REPORT_b6225600860cc46d.md"
    
    if not ultra_report_path.exists():
        print(f"ERRO: Relatório ultra completo não encontrado: {ultra_report_path}")
        return False
    
    with open(ultra_report_path, 'r', encoding='utf-8') as f:
        ultra_content = f.read()
    
    # Extrair informações do relatório
    report_id_match = re.search(r'\*\*Report ID:\*\* `([a-f0-9]+)`', ultra_content)
    timestamp_match = re.search(r'\*\*Generated:\*\* ([^\n]+)', ultra_content)
    
    report_id = report_id_match.group(1) if report_id_match else "b6225600860cc46d"
    timestamp = timestamp_match.group(1) if timestamp_match else "2025-12-15 00:58:43 CET"
    
    # Criar relatório executivo limpo (sem emojis problemáticos)
    executive_report = f"""# AURORA PROJECT - RELATORIO EXECUTIVO
...
- **Arquivo:** `AURORA_ULTRA_COMPLETE_REPORT_{report_id}.md`
- **Localizacao:** `05-Documentacao/Audit-Reports/`
- **Caminho Completo:** `C:\\Users\\Lenovo\\Projects\\Aurora\\05-Documentacao\\Audit-Reports\\AURORA_ULTRA_COMPLETE_REPORT_{report_id}.md`

**Link Direto:**

```
file:///C:/Users/Lenovo/Projects/Aurora/05-Documentacao/Audit-Reports/AURORA_ULTRA_COMPLETE_REPORT_{report_id}.md
```
...
"""
    ...
    # Salvar o relatório executivo restaurado
    output_path = Path(__file__).parent.parent / "RELATORIO_EXECUTIVO_CONFLITOS_INTERESSE.md"
    
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(executive_report)
    
    print(f"Relatorio executivo restaurado com sucesso!")
