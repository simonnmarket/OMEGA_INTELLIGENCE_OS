...
#include <include/intelligence/adaptive_learning.mqh>
...
private:
   ...
   double CalculateFitness(double &weights[])
   {
      ...
      // Fitness = ProfitFactor - Drawdown (maximizar)
   ...
   bool EvolvePopulation()
   {
      ...
      for(int gen = 0; gen < GENERATIONS && !IsStopped(); gen++)
      {
         ...
         // Seleção (elitismo: mantém o melhor)
         int best_index = 0;
