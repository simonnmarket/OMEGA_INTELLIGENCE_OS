...
//+------------------------------------------------------------------+
#ifndef __AUDIT_ISSUE_ENUM_MQH__
#define __AUDIT_ISSUE_ENUM_MQH__
...
public:
   ...
   //+--------------------------------------------------------------+
   //| Obtém descrição detalhada do problema                         |
   //+--------------------------------------------------------------+
   string GetDescription(ENUM_AUDIT_ISSUE issue)
   {
      switch(issue)
      {
         case AUDIT_ISSUE_INVALID_NAME: 
            return "Nome de arquivo não segue padrão de nomenclatura";
         case AUDIT_ISSUE_MISSING_HASH: 
            return "Arquivo sem proteção SHA3-256";
         case AUDIT_ISSUE_MISSING_BLOCKCHAIN: 
            return "Arquivo não registrado no blockchain";
         case AUDIT_ISSUE_CIRCULAR_DEPENDENCY: 
            return "Dependência circular detectada";
         case AUDIT_ISSUE_SECURITY_VULNERABILITY: 
            return "Vulnerabilidade de segurança identificada";
         case AUDIT_ISSUE_UNDECLARED_VARIABLE: 
            return "Variável utilizada sem declaração";
         case AUDIT_ISSUE_INVALID_INCLUDE: 
            return "Include de arquivo inexistente ou inválido";
         case AUDIT_ISSUE_DEPRECATED_FUNCTION: 
            return "Função obsoleta utilizada";
         case AUDIT_ISSUE_PERFORMANCE_ISSUE: 
            return "Problema de performance detectado";
         case AUDIT_ISSUE_MEMORY_LEAK: 
            return "Possível vazamento de memória";
         case AUDIT_ISSUE_THREAD_SAFETY: 
            return "Problema de thread safety";
         case AUDIT_ISSUE_MISSING_VALIDATION: 
            return "Falta validação de entrada";
         case AUDIT_ISSUE_INVALID_PARAMETER: 
            return "Parâmetro inválido passado";
         case AUDIT_ISSUE_MISSING_ERROR_HANDLING: 
            return "Falta tratamento de erro";
         case AUDIT_ISSUE_INCONSISTENT_NAMING: 
            return "Nomenclatura inconsistente";
         default: 
   ...
   //+--------------------------------------------------------------+
   //| Obtém sugestão de correção                                    |
   //+--------------------------------------------------------------+
   string GetSuggestedFix(ENUM_AUDIT_ISSUE issue)
   {
      switch(issue)
      {
         case AUDIT_ISSUE_INVALID_NAME: 
            return "Renomear arquivo seguindo padrão: nome_modulo.mqh";
         case AUDIT_ISSUE_MISSING_HASH: 
            return "Adicionar SHA3-256 no cabeçalho do arquivo";
         case AUDIT_ISSUE_MISSING_BLOCKCHAIN: 
            return "Registrar arquivo no blockchain de integridade";
         case AUDIT_ISSUE_CIRCULAR_DEPENDENCY: 
            return "Reestruturar dependências para evitar ciclos";
         case AUDIT_ISSUE_SECURITY_VULNERABILITY: 
            return "Implementar validações de segurança";
         case AUDIT_ISSUE_UNDECLARED_VARIABLE: 
            return "Declarar variável antes do uso";
         case AUDIT_ISSUE_INVALID_INCLUDE: 
            return "Corrigir caminho do include ou criar arquivo";
         case AUDIT_ISSUE_DEPRECATED_FUNCTION: 
            return "Substituir por função atualizada";
         case AUDIT_ISSUE_PERFORMANCE_ISSUE: 
            return "Otimizar algoritmo ou estrutura de dados";
         case AUDIT_ISSUE_MEMORY_LEAK: 
            return "Implementar liberação adequada de memória";
         case AUDIT_ISSUE_THREAD_SAFETY: 
            return "Implementar sincronização adequada";
         case AUDIT_ISSUE_MISSING_VALIDATION: 
            return "Adicionar validações de entrada";
         case AUDIT_ISSUE_INVALID_PARAMETER: 
            return "Validar parâmetros antes do uso";
         case AUDIT_ISSUE_MISSING_ERROR_HANDLING: 
            return "Implementar tratamento de exceções";
         case AUDIT_ISSUE_INCONSISTENT_NAMING: 
            return "Padronizar nomenclatura do projeto";
         default: 
            return "Analisar problema específico";
      }
   }

   //+--------------------------------------------------------------+
   //| Obtém severidade recomendada                                  |
   //+--------------------------------------------------------------+
...
#endif // __AUDIT_ISSUE_ENUM_MQH__ 