...
#include <include/quantum/quantum_blockchain.mqh>
...
private:
   ...
   bool is_valid_context()
   {
      ...
      {
         m_logger.log_error("[RM] Sem conexão com o servidor de mercado");
         return false;
      }

      if(!m_logger.is_initialized())
      {
         m_logger.log_error("[RM] Logger não inicializado");
         return false;
      }

      if(!m_blockchain.IsReady())
      {
         m_logger.log_warning("[RM] Blockchain quântico não está pronto");
         return false;
      }

      if(StringLen(m_symbol) == 0 || !SymbolInfoInteger(m_symbol, SYMBOL_SELECT))
      {
         m_logger.log_error("[RM] Símbolo inválido: " + m_symbol);
         return false;
      }
   ...
   double CalculateMaxDrawdown()
   {
      ...
      if(copied <= 0)
      {
         m_logger.log_error("[RM] Falha ao copiar preços para cálculo de drawdown");
         return 0.0;
      }
   ...
   void updateRiskDisplay(double var, double dd)
   {
      ...
      {
         m_risk_label = new CLabel("RiskLabel", 0, 10, 1200);
         m_risk_label->text("RISCO: ????");
         m_risk_label->color(clrGray);
      }

      if(m_var_value == NULL)
      {
         m_var_value = new CLabel("VarValue", 0, 10, 1220);
         m_var_value->text("VAR: 0%");
         m_var_value->color(clrGray);
      }
...
public:
      ...
      m_last_calculation_time(0)
   {
      if(!m_logger.is_initialized())
      {
         Print("[RM] Logger não inicializado");
         ExpertRemove();
      }

      if(!m_blockchain.IsReady())
      {
         m_logger.log_error("[RM] Blockchain quântico não está pronto");
         ExpertRemove();
      }
      ...
      m_logger.log_info("[RM] Módulo de métricas de risco inicializado para " + m_symbol);
   }
   ...
   VaRResult CalculateVaR(ENUM_VAR_METHOD method = VAR_HISTORICAL)
   {
      if(!is_valid_context())
      {
         m_logger.log_error("[RM] Contexto inválido. Cálculo de VaR bloqueado.");
      ...
      m_logger.log_info("[RM] VaR calculado - Valor: " + DoubleToString(var*100, 2) + 
   ...
   void RecordCalculation(const VaRResult &result)
   {
      ...
      m_blockchain.RecordData(data, "RISK_CALCULATION");
      m_logger.log_info("[RM] Cálculo registrado na blockchain: " + data);
   }
   ...
   double GetVaR95()
   {
      VaRResult result = CalculateVaR(VAR_HISTORICAL);
      return result.value_at_risk;
   }

   //+--------------------------------------------------------------+
   //| Obtém Expected Shortfall                                   |
   //+--------------------------------------------------------------+
   double GetExpectedShortfall()
   {
      VaRResult result = CalculateVaR(VAR_HISTORICAL);
   ...
   //+--------------------------------------------------------------+
   //| Obtém Drawdown Máximo                                        |
   //+--------------------------------------------------------------+
   double GetMaxDrawdown()
   {
      return m_max_drawdown > 0 ? m_max_drawdown : CalculateMaxDrawdown();
   }
   ...
   bool IsReady() const
   {
      return m_logger.is_initialized() && m_blockchain.IsReady();
   }
   ...
   bool ExportMetricsHistory(string file_path)
   {
      ...
      FileClose(handle);
      m_logger.log_info("[RM] Histórico de métricas exportado para: " + file_path);
      return true;
   }
};
