...
private:
   ...
   {
      if(m_anomaly_label == NULL) m_anomaly_label = new CLabel();
   }
...
public:
      ...
      : m_logger(logger), m_data_stream(data_stream), m_quantum_entropy(quantum_entropy), m_symbol(symbol), m_entropy_threshold(entropy_threshold), m_is_initialized(false), m_last_train_time(0), m_last_detection_time(0)
   {
      ...
      if(StringLen(m_symbol) == 0 || !SymbolInfoInteger(m_symbol, SYMBOL_SELECT)) { if(m_logger) m_logger->log_error("[ANOMALY] Símbolo inválido: " + m_symbol); return; }
