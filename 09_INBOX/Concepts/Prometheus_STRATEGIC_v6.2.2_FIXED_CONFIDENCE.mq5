...
#property description "Prometheus STRATEGIC v6.2.1 - Ultra Conservative Mode + MarketPsyEngine"

#include <Trade/Trade.mqh>
#include <MarketPsyEngine.mqh>  // TASK 1.3: Motor de Sentimento Integrado

// =============================================================================
// CONFIGURAÇÕES ULTRA-CONSERVADORAS - QUALIDADE SOBRE QUANTIDADE
// =============================================================================
input bool   UseMarketWatch        = false;  // MODIFICADO: usar lista fixa   // STRATEGIC: Multi-asset controlado
...
// TASK 1.3: Instância global do Motor de Sentimento
...
// =============================================================================
int OnInit() {
   Print("[PROMETHEUS_STRATEGIC_V6.2.1] Online com MarketPsyEngine. FilesDir=", FilesDir());
   
   // TASK 1.3: Inicializar Motor de Sentimento
   if(g_SentimentEngine == NULL) {
      ...
      if(g_SentimentEngine != NULL) {
         ...
         Print(g_SentimentEngine.GetProductionStatus());
      } else {
   ...
   Print("[PROMETHEUS_STRATEGIC_V6.2.1] Sistema STRATEGIC + MarketPsyEngine inicializado com sucesso");
   return INIT_SUCCEEDED;
}

void OnDeinit(const int reason) {
   Print("[PROMETHEUS_STRATEGIC_V6.2.1] Stopped. Reason=", reason);
   
   // TASK 1.3: Liberar Motor de Sentimento
   if(g_SentimentEngine != NULL) {
...
void ProcessCycle() {
   ...
   if(UseMarketWatch) {
      ...
      while(processed < MaxSymbolsPerCycle && i < total) {
         ...
         if(SymbolAllowed(sym)) {
            ...
            if(TryReadResponse(sym, action, conf)) {
               // TASK 1.3: Ajustar confiança com sentimento de mercado
               double conf_adjusted = ProcessSentimentAndAdjustConfidence(sym, conf);
               
               Print("[RESPONSE] ", sym, ": ", action, " conf=", DoubleToString(conf_adjusted, 3), 
                     " (min: ", DoubleToString(minConf, 2), ")");
               
               if(IsSpreadOkBase(sym, spreadPts) && conf_adjusted >= minConf && (action == "BUY" || action == "SELL")) {
                  ExecuteSignal(sym, action, conf_adjusted, slm, tpm, trm, minEdgeBps);
               } else {
                  if(!IsSpreadOkBase(sym, spreadPts)) {
                     Print("[REJECTED] ", sym, " - Spread muito alto");
                  } else if(conf_adjusted < minConf) {
                     Print("[REJECTED] ", sym, " - Confiança ajustada baixa: ", DoubleToString(conf_adjusted, 3));
      ...
      // USAR LISTA FIXA DE SÍMBOLOS VALIDADOS
...
// TASK 1.3: Função para processar sentimento e ajustar confiança
double ProcessSentimentAndAdjustConfidence(const string symbol, double python_conf) {
   ...
   // Coletar dados de microestrutura
   ...
   double bid_volume = 0.0; // Level 2 não disponível
   double ask_volume = 0.0; // Level 2 não disponível
   double empty_array[]; // Arrays vazios para market depth
   
   // Processar através do motor de sentimento
   if(!g_SentimentEngine.ProcessMarketData(current_price, current_volume, current_spread,
      ...
      return python_conf; // Fallback se processar falhar
   }
   ...
   // AJUSTAR CONFIANÇA: média ponderada (70% Python + 30% Sentiment)
